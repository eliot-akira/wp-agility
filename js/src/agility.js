...
//              $node.siblings('input[name="'+nodeName+'"]').filter('[value="'+modelValue+'"]').prop("checked", true); // this won't fire a DOM 'change' event, saving us from an infinite event loop (Model <--> DOM)
    ...
    replaceChildren: function( proto, models, selector ) {
  ...
  agility = function(){
    ...
    // @extend If a single string is given, set it as view.format
...
/*
  ...
  // WordPress adapter using jQuery's ajax()
  agility.adapter.wp = function(_params){
